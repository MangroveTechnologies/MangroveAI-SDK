...
class MockTransport:
    ...
    Usage:
        mock = MockTransport()
        mock.add_response("GET", "/strategies/", json={"strategies": [], "total": 0})
        # Pass mock._transport or use mock directly
    """
    ...
    def request(
        ...
        *,
        ...
        return TransportResponse(
            status_code=404,
            headers={},
            data={"error": "not_found", "message": f"No mock for {method} {url}", "code": "MOCK_NOT_FOUND"},
            text=json_mod.dumps({"error": "not_found", "message": f"No mock for {method} {url}"}),
        )
